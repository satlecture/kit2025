...
\author{{Markus Iser}, \underline{Dominik Schreiber}}
\date{June 16, 2025}
...
\begin{frame}{Explicit Partitioning}
	...
	\textbf{Böhm \& Speckenmeyer (1994)}: 1st Parallel DPLL Implementation\\[3mm]
...
\begin{frame}{Explicit Partitioning}
	...
	``\textbf{PSATO}: a Distributed Propositional Prover and its Application to Quasigroup Problems'', Zhang et al., 1996\\[3mm]
...
\begin{frame}{Explicit Partitioning Solvers}
	...
	\begin{blueblock}{{SATZ} (Jurkowiak et al., 2001): \emph{Work stealing} for workload balancing}
		\begin{itemize}
...
\begin{frame}{Problems with Partitioning}
...
\begin{frame}{Cube and Conquer}
	\begin{blueblock}{The Cube\&Conquer paradigm (Heule \& Biere, 2011)}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Used to solve the ``Pythagorean Triples'' problem ($\sim$200TB proof)
			\item ... or more recently ``Schur Number 5'' ($\sim$2PB proof)
		\end{itemize}
		\item Examples: March (Heule) + iLingeling (Biere) introduced in 2011; Treengeling (Biere)
	\end{itemize}
\end{frame}
...
\begin{frame}{Pure Portfolio: Oracle view vs. Speedup view}
	...
	\vspace*{0.2cm}
	\pause
	\begin{blueblock}{Parallel speedup}
...
\begin{frame}{Pure SAT Portfolios}
	\begin{blueblock}{ppfolio: Winner of Parallel Track in the 2011 SAT Competition}
		\begin{itemize}
...
\begin{frame}{Early Clause Sharing Portfolios}
	...
	\begin{blueblock}{ManySAT (Hamadi, Jabbour, and Sais 2009)}
		\begin{itemize}
	...
	\end{blueblock}
	\pause
	\begin{blueblock}{Plingeling (Biere 2010)}
		\begin{itemize}
...
\begin{frame}{Massively parallel hardware?}
	...
	\begin{minipage}{0.7\textwidth}
		\begin{itemize}
			...
			\pause
			\vspace*{1mm}
			\item Some exchange schemes are \highlo{conceptually not scalable}
			\begin{itemize}
...
\begin{frame}{Massively parallel SAT portfolio}
	\begin{blueblock}{HordeSat (Balyo, Sanders, Sinz 2015)}
		\begin{itemize}
...
\begin{frame}{HordeSat: Results}
	...
	\begin{minipage}{0.5\textwidth}
		...
		\centering \unimp{Data extracted from HordeSat paper}
	\end{minipage}%
\end{frame}
...
\begin{frame}{From HordeSat to MallobSat}
	...
	\begin{blueblock}{Result: Mallob}
		...
		\begin{itemize}
			\item multi-user, \highl{on-demand}, malleable scheduling and solving of \highl{many problems at once}
			\item distributed SAT engine \textbf{MallobSat}: the HordeSat paradigm \highl{re-engineered and made efficient}
			\item state-of-the-art SAT performance \highl{from dozens to thousands of cores}
		\end{itemize}
	\end{blueblock}
	\vspace*{7mm}
	\begin{minipage}{0.5\textwidth}
...
\begin{frame}{Design Decisions of \textsc{MallobSat}}
	\begin{minipage}{0.45\textwidth}
		\begin{itemize}
...
\begin{frame}{Clause Exchange in Mallob}
	\begin{minipage}{0.43\textwidth}
		\textbf{Custom collective operation} \unimp{[SAT'21]}
		\begin{itemize}
...
\begin{frame}{Clause Filtering}
	\begin{minipage}{0.45\textwidth}
		...
		\end{blueblock}
		\vspace*{0.1cm}
		\textbf{Previously:} \unimp{[HordeSat] [SAT'21]}
		\begin{itemize}
		...
		\vspace*{0.1cm}
		\pause
		\textbf{Mallob'22+:} \highl{Exact distributed filter} \unimp{[ISC'22]}
		\begin{itemize}
...
\begin{frame}{Enforcing a Sharing Volume}
...
\begin{frame}{Handling LBD Values}
	\begin{minipage}{0.6\textwidth}
		...
		\vspace{0.3cm}
		\pause
		Sensible approach: \highl{Increment each LBD before import}
		\begin{itemize}
		...
		\phantom{\textbf{But:}} in latest experiments
		}
	\end{minipage}%
	\begin{minipage}{0.4\textwidth}
		\centering
...
\begin{frame}{Scaling of MallobSat}
...
\begin{frame}{Diversification vs.\ Sharing @ 768 Cores\ \ \unimp{[JAIR'24]}}
...
\begin{frame}{MallobSat: A Portfolio Solver?}
	...
	\begin{itemize}
		\item ``\textit{a problem instance is independently given to a collection of solvers \highlo{competing} for a solution in parallel}''\\\hfill\unimp{-- Fichte et al., 2023}
		\item ``\textit{each thread runs a \highlo{different SAT solver} on the same instance[, which] \highlo{in combination with} clause-sharing leads to surprisingly good performance \highlo{for small portfolio sizes}}''\hfill\unimp{-- Ozdemir et al., 2021}
	\end{itemize}
...
\begin{frame}{Better Efficiency?}
	\begin{minipage}{0.65\textwidth}
		...
		\vspace{0.2cm}
		\pause
		\textbf{Best of both worlds?} \ \ \unimp{[EuroPar'22]}
		\begin{itemize}
...
\begin{frame}{Scheduling Many SAT Tasks\ \ \unimp{[JAIR'24]}}
	\vspace*{-1cm}
...
\begin{frame}{Current Frontiers}
	\begin{itemize}
		\item Streamlined \highl{clause-sharing solver} design, away from portfolios
		\item Address \highlo{high main memory requirements}, especially for huge formulas
		\item Preprocessing and Inprocessing in parallel SAT solving
		...
		\\-- Crucial piece of technology: Distributed \highl{incremental SAT solving}!
		\item \highlo{Uses of GPUs ??} (Stochastic Local Search, Inprocessing offloading, \ldots)
	\end{itemize}
\end{frame}
