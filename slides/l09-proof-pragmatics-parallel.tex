...
\begin{frame}{DRUP}
	\begin{blueblock}{Approach 3: DRUP (Deletion RUP) Proof}
		\vspace*{2mm}\hspace*{0.5cm}\begin{minipage}{10cm}
			$B := F$\\
...
\begin{frame}{Proof (File) Formats: DRAT and LRAT}
	...
	\vspace*{0mm}
	\centering
	\includegraphics[height=9cm]{figures/l09/proof-formats.pdf}\\[2mm]
...
\textbf{Results~\cite{michaelson2025producing}:}
\\[2mm]
\begin{itemize}
...
\hspace*{-6mm}\begin{minipage}[c][8cm][t]{0.95\textwidth}
	\begin{itemize}
		...
		\pause
		\\-- \highlo{Cannot check external clause} because its prerequisites are unknown and (probably) not even present
		...
		\pause
		\vspace*{5mm}
		\item Checkers \highl{sign} each successfully checked clause $c$ with \highl{cryptographic signature} based on
		\highl{shared secret $K$}:\\[1mm]
		$\mathcal{S}_K(c) = H_K(\texttt{LRUP\_ID}(c)\ ||\ c\ ||\ \mathcal{S}_K(F))$
		\\-- $H_K$: \highl{Message Authentication Code} (MAC),	specifically 128-bit \textbf{SipHash}~\cite{aumasson2012siphash}
		\item Clauses are shared \highl{together with their signatures}
...
\begin{frame}{Real-time Checking: Full Setup}
	...
	\uncover<7->{
		\\[1.5cm]
	$\Rightarrow$ \textbf{We \highl{only} need to trust our signatures and our parser, checkers, and confirmer.}
	\\\phantom{$\Rightarrow$} Not the SAT solvers, not the sharing logic, not the communication, \ldots
	}
\end{frame}

\begin{frame}{Real-time Checking: Scalability\ \ \cite{schreiber2024trusted}}
